...
=head1 NAME
	...
	ppt_getGI.pl -- returns GI numbers from a user specified NCBI database and search term
...
=head2 Usage
	...
	perl ppt_getGI.pl -db <name of the database as it appears in NCBI(protein|nucleotide)> -o <output file>
...
$params{db} = $db;
$params{tool} = "ebot";
$params{term} = "Viruses\[Organism\] AND srcdb_refseq\[PROP\] NOT cellular organisms\[ORGN\] NOT AC_000001:AC_999999\[pacc\] AND gbdiv_phg\[properties\]";
%params = esearch(%params);

$params{outfile} = $out;
@uids = get_uids(%params);
open(OUTFILE, ">$params{outfile}") || die $!;
